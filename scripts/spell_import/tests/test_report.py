...
LOCK = provenance.SourceIdentity(
    book_id="arm5-core", book="Book", path="reviewed/Book.md", sha256="a" * 64,
...
CURRENT = provenance.SourceIdentity(
    book_id="arm5-core", book="Book", path="reviewed/Book.md", sha256="b" * 64,
...
class RenderTest(unittest.TestCase):
    ...
    def _identity(self, book_id, book, sha="a" * 64, parsed=10, imported=9):
        return provenance.SourceIdentity(
            book_id=book_id, book=book, path=f"reviewed/{book}.md",
            sha256=sha, rulebook=None, spells_parsed=parsed, spells_imported=imported)

    def test_header_names_both_revisions_and_the_count_transitions(self):
        text = report.render(
            self.diff, locks={LOCK.book_id: LOCK}, currents=[CURRENT],
            imported=250, blocked=110, unresolved=0)
    ...
    def test_no_lock_renders_as_an_initial_import(self):
        text = report.render(
            self.diff, locks={}, currents=[CURRENT],
            imported=250, blocked=110, unresolved=0)
