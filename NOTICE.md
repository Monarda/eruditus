...
Eruditus is licensed in two halves: rulebook-derived content is Adapted Material under CC BY-SA 4.0 wherever it appears in this repository, and everything else is MIT. That currently includes assets/data, scripts/spell_import/resolutions.json, scripts/spell_import/hand_authored_templates.json and scripts/spell_import/container_modes.json. See LICENSE and NOTICE.md.

The table below names the paths that are wholly one or the other, so the
boundary stays checkable — it is illustrative, not an exhaustive enumeration
of every file that carries rulebook text.
