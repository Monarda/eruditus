...
- [ ] **Add the 2 still-missing modifiers to `modifiers.json`** — Creo Aquam
      ...
      treatment row.
- [ ] **Collapse the duplicated level sum in `asset_data_loader_test.dart`**
      (still present 2026-08-17, `:322-340`). Its "every loaded spell calculates
      to the level stated in its description" builds its own magnitude list
      rather than calling `SpellEngine.calculateBreakdown`, and item 24 had to
      add `spell.adjustments` to it. Two hand-maintained copies of the same sum
      will drift again; `published_spell_import_test.dart`'s assertion 1 already
      calls the engine.
- [ ] **`catalog._STOPWORDS` still contains `"phantasm"`,** a content word, not a
      stopword (`catalog.py:26`). **Not a harmless no-op:** three Phantasm spells
      import with it stripped from their ids — `lib-crim-human-form` and
      `lib-crim-talking-head` both lost it; `lib-crim-phantasmal-animal` kept it
      only because "Phantasmal" is not the listed token. Removing it renames two
      committed ids, so this is an asset change with migration weight.
...
### 6. Widget-Test Coverage Hole from the Real-Bloc Hang
...
- **✅ Corrected 2026-08-17: the suite is green, not red.** This item carried a
  "⚠️ 2 of 5 fail" note from 2026-08-06 (two `scrollUntilVisible` failures). Those
  were fixed on 2026-08-10 with a `maxScrolls: 500` budget, and a run today
  (`flutter test integration_test/spell_creation_flow_test.dart -d windows`)
  passes **8 of 8**. The checklist above is what remains.
- **Two failure modes that still bind:**
