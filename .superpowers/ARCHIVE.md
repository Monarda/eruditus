...
- [x] **80.1** **Stand up the mechanism**: `flutter_localizations` + `intl`,
      `l10n.yaml`, `generate: true`, one `app_en.arb`, and `MaterialApp`'s
      `localizationsDelegates`/`supportedLocales`.
- [x] **80.2** **Migrate the existing literals**, and add a guard that stops new bare
...
- [x] **80.3** **⚠️ Decide how the text populations localise differently — the one
...
- [x] **80.4 (Task 11)** **Stand up the pseudo-locale coverage guard** — DONE
      2026-08-20. `test/l10n/pseudo_locale_coverage_test.dart` pumps each of the
      four top-level screens (Create, Library, Configuration, Backup — there is
      no way to pump `main.dart`'s private `_MainTabView` directly) under
      `Locale('en', 'XA')` and asserts a fixed chrome-string list is unfindable,
      plus asserts the four realm values *are* findable, keeping the
      chrome/content boundary itself under test. 781 Dart tests green (up from
      745), 397 Python green, 8 integration green, `flutter analyze` clean.
...
- **Files:** `pubspec.yaml`, new `l10n.yaml` + `lib/l10n/app_en.arb`,
  ...
  holding 47 inline `MaterialApp` constructions, plus
  `test/l10n/pseudo_locale_coverage_test.dart` (Task 11)
- **See also:** items 56 (the string-volume driver), 79 (why the boundary
  ...
  that item makes), 81 (Latin, blocked on this item), 82 (the MT-provenance
  flag, folded in narrowly)
